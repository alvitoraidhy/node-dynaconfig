...
const filePath = {
  async: path.join(fixtureFolder, "example.async.env"),
};
...
test("Should run the example usage correctly (async)", async () => {
  const envPath = path.resolve(__dirname, filePath.async);
  const md5hash = (str) => crypto.createHash("md5").update(str).digest("hex");

  // ------------------------ EXAMPLE START ------------------------
  // ...
  const { async: AsyncEnvStore } = require("../src");
  ...
  const fileContent = await fs.promises.readFile(envPath);
