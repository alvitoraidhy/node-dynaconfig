...
const filePath = {
  async: path.join(fixtureFolder, "example.async.yaml"),
};
...
test("Should run the example usage correctly (async)", async () => {
  const yamlPath = path.resolve(__dirname, filePath.async);
  const md5hash = (str) => crypto.createHash("md5").update(str).digest("hex");

  // ------------------------ EXAMPLE START ------------------------
  // ...
  const { async: AsyncYamlStore } = require("../src");
  ...
  const fileContent = await fs.promises.readFile(yamlPath);
