...
const filePath = {
  async: path.join(fixtureFolder, "example.async.json"),
};
...
test("Should run the example usage correctly (async)", async () => {
  const jsonPath = path.resolve(__dirname, filePath.async);
  const md5hash = (str) => crypto.createHash("md5").update(str).digest("hex");

  // ------------------------ EXAMPLE START ------------------------
  // ...
  const { async: AsyncJsonStore } = require("../src");
  ...
  const fileContent = await fs.promises.readFile(jsonPath);
