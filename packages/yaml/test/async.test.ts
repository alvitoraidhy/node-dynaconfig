...
import { async as AsyncYamlFile } from "../src";
...
describe("Asynchronous Dynaconfig (yamlfile)", () => {
  test("Should load the relative 'config.yaml' by default", async () => {
    const currentDir = process.cwd();
    process.chdir(path.resolve(__dirname, fixtureFolder));
    const config = new AsyncYamlFile();
  ...
  test("Should load the given '.yaml' path", async () => {
    const config = new AsyncYamlFile(
  ...
  test("Should create a new file if the file does not exist", async () => {
    ...
    const config = new AsyncYamlFile(filePath);
