...
import { async as AsyncTomlFile } from "../src";
...
describe("Asynchronous Dynaconfig (tomlfile)", () => {
  test("Should load the relative 'config.toml' by default", async () => {
    const currentDir = process.cwd();
    process.chdir(path.resolve(__dirname, fixtureFolder));
    const config = new AsyncTomlFile();
  ...
  test("Should load the given '.toml' path", async () => {
    const config = new AsyncTomlFile(
  ...
  test("Should create a new file if the file does not exist", async () => {
    ...
    const config = new AsyncTomlFile(filePath);
